...
<?php
  ...
  if($mode=="modify"){
    ...
    if($mode=="response"){
      ...
      $sql.=" values($group_num, $depth, $ord, '$userid', '$username', '$usernick', '$subject', '$content', now(), 0, '$is_html')";
      mysql_query($sql, $connect);
    }else{// 일반 인설트
      $depth=0;
      $ord=0;
      ...
      $sql.=" values($depth, $ord, '$userid', '$username', '$usernick', '$subject', '$content', now(),0,'$is_html')";
      mysql_query($sql, $connect);
