...
<div class="view_form">
				...
				<form name="ripple_form" method="post" action="re_insert.php?table=<?=$table?>&ripple=<?=$ripple?>&br_num=<?=$item_num?>&page=<?=$page?>">
					<div id="ripple_insert">
						<div id="ripple_textarea" class="text" >
							<textarea id="ripple_content" name="ripple_content"></textarea>
							<span id="remaining">(<span id="count">0</span>/<span id="max">300</span>)</span>
						</div>
	...
	<div id="button">
		...
		?>
		<a href="write.php?table=<?=$table?>&mode=modify&br_num=<?=$br_num?>&page=<?=$page?>&mypage=<?=$mypage?>&test=<?=$test?>">수정하기</a>
		<a href="javascript:del('delete.php?table=<?=$table?>&br_num=<?=$br_num?>&page=<?=$page?>&mypage=<?=$mypage?>&test=<?=$test?>')">삭제하기</a>
		<?
			}
		?>
	</div>
