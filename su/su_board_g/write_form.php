<?php
  ...
  if($mode=="modify"){
    ...
    $item_subject=$row[bg_subject];
    $item_content=$row[bg_content];
    $item_file_0=$row[bg_file_name_0];
    $item_file_1=$row[bg_file_name_1];
    $item_file_2=$row[bg_file_name_2];
    
    $copied_file_0=$row[bg_file_copied_0];
    $copied_file_1=$row[bg_file_copied_1];
    $copied_file_2=$row[bg_file_copied_2];
  }
?>
  <script>
  ...
  <div class="write_form">
	...
	<?php } ?>
		...
		<table>
			...
			<?if ($copied_file_0==""&&$copied_file_1==""&&$copied_file_2==""&&$mode=="modify"){?>
                  <input type="button" value="목록보기" onclick="check_input();"/>
				 <?}else{?>
