...
/* 페이징 시작 */
	...
	$paging = '<ul>'; // 페이징을 저장할 변수
		...
		//첫 페이지가 아니라면 처음 버튼을 생성
		if($page != 1) { 
			$paging .= '<li class="page page_start"><a href="./member_list.php?page=1' . $subString . '">처음으로</a></li>';
		}
		//첫 섹션이 아니라면 이전 버튼을 생성
		if($currentSection != 1) { 
			$paging .= '<li class="page page_prev"><a href="./member_list.php?page=' . $prevPage . $subString . '">이전</a></li>';
		}
		...
		for($i = $firstPage; $i <= $lastPage; $i++) {
			if($i == $page) {
				...
				$paging .= '<li class="page"><a href="./member_list.php?page=' . $i . $subString . '">' . $i . '</a></li>';
			}
		}
		
		//마지막 섹션이 아니라면 다음 버튼을 생성
		if($currentSection != $allSection) { 
			$paging .= '<li class="page page_next"><a href="./member_list.php?page=' . $nextPage . $subString . '">다음</a></li>';
		}
		
		//마지막 페이지가 아니라면 끝 버튼을 생성
		if($page != $allPage) { 
			$paging .= '<li class="page page_end"><a href="./member_list.php?page=' . $allPage . $subString . '">마지막페이지</a></li>';
		}
		$paging .= '</ul>';
