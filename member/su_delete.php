    ...
    include "../../common.php";		
		...
		$total_record=mysql_num_rows($result);
			if($total_record>0){
				...
				echo "<script>window.alert('".$total_date."취소후 탈퇴가 가능합니다.');history.go(-1);</script>";
			}else{
				...
				if($userid == $m_id){include _BASE_DIR."/member/logout.php";}
				echo "<script>window.alert('탈퇴 처리가 완료되었습니다.');history.go(-1);</script>";
			}
			mysql_close();
	}	
